...
class ThetaStar:
    ...
    def UpdateVertex(self, s, succ):
        if self.LineOfSight(s.parent, succ):
            # Path 2
            # if s.parent.g + s.parent.getWeight(succ) < succ.g:
            if s.parent.g + math.dist(s.parent.getKeyCoordinates(), succ.getKeyCoordinates()) < succ.g :
                succ.setG(s.parent.g + math.dist(s.parent.getKeyCoordinates(), succ.getKeyCoordinates()))
